...
a sub-sequence of tokens of text $ T $ of length $ i+1 $ starting at index $ j $.
...
The method is computationally rather heavy (in the naive case).
...
That is to say, $ SDC(A,B) >= M $ in order to pronounce $ B $ a match to $ A $.
