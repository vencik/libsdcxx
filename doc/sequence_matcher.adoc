...
For $ i > 0 $, we shall prove that $ B_{i,j} $ defined as above is an union of bigrams
multi-set of text tokens of length $ i + 1 $ starting at index $ j $.

As the induction predicate, we assume that it is the case for all $ 0 le k < i $.
